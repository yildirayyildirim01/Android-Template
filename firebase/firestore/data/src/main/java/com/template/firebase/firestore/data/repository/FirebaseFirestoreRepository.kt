...
class FirebaseFirestoreRepository @Inject constructor(
    ...
    override fun collection(path: String): CollectionReference = firebaseFirestore.collection(path)

    override fun document(path: String): DocumentReference = firebaseFirestore.document(path)

    override suspend fun getDocument(path: String): DocumentSnapshot = document(path).get().await()

    override suspend fun getCollection(path: String): QuerySnapshot = collection(path).get().await()
